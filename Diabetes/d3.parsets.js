    ...
    function parsets(selection) {
      selection.each(function(data, i) {
        ...
        data.forEach(function(d) {
          dimensionNames.forEach(function(e) { cats.add(d[e]); });      
        });     
        
        var nums = [];        
        var categories = ["Missing", "Death", "Uncontrolled", "ControlledDiab", "BorderlineDiab", "Normal"];    
        
        cats.forEach(function(d) { 
          if (categories.indexOf(d) === -1) {
            var v = +d;
            if (v) nums.push(v);
            else categories.push(d);
          } 
        });
        
        categories = categories.concat(nums.sort(d3.descending));
...
//        d3.select(window).on("mousemove.parsets." + ++parsetsId, unhighlight);
        ...
        function createGradients() {   
          // XXX: Copied from d3.parsets.css, would be nice if this was automatic...
          var colors = [ "#999", "#333", "#d62728", "#ff7f0e", "#1f77b4", "#2ca02c", "#cccc28", "#984ea3" ];
    
          for (var i = 0; i < colors.length; i++) {
            for (var j = 0; j < colors.length; j++) {
              var gradient = g.append("linearGradient")
                  .attr("id", "gradient" + (i) + (j))
                  .style("color-interpolation", "linearRGB")  // XXX: Does this do anything
                  .attr("x1", "0%")
                  .attr("y1", "0%")
                  .attr("x2", "0%")
                  .attr("y2", "100%");
...
//          dimensions.sort(compareY); //Hard coded the sorting by Hina
...
/*                                         
          var catScale = d3.scale.ordinal().domain(c).range(d3.range(c.length));
          dimensions.forEach(function(d) {
            d.categories.sort(function(a, b) { return d3.ascending(cScale(a.name), cScale(b.name)); });
          });
*/          
          // XXX: Hard code domain...
          ordinal
            //.domain(["Missing", "Death", "Uncontrolled", "ControlledDiab", "BorderlineDiab", "Normal", "Male", "Female"])
	  ...
	  nodes = layout(tree, dimensions, ordinal);
          ...
          var textEnter = dEnter.append("text")
              .attr("class", "dimension")
              .attr("transform", "translate(10,0)rotate(-21)")
        ...
        function updateRibbons() {
          ...
          var cats = ["Missing", "Death", "Uncontrolled", "ControlledDiab", "BorderlineDiab", "Normal", "Male", "Female"];
          var categoryNumber = d3.scale.ordinal().domain(cats).range(d3.range(cats.length));
          
          if(ribbon.node() != undefined)
          { //For updating the color for ribbon
            ribbon            
                .attr("class", function(d) { return "category-" + (d.major); })                 
                .attr("d", ribbonPath)
                .style("fill", function(d) { 
                  if ((col_trans.checked && !flip.checked) || (col_reverse.checked && flip.checked)) {
                      return "url(#gradient" + (categoryNumber(d.source.node.name)) + (categoryNumber(d.target.node.name));
                  }
                  else if ((col_reverse.checked && !flip.checked) || (col_trans.checked && flip.checked)) {
                      return "url(#gradient" + (categoryNumber(d.target.node.name)) + (categoryNumber(d.source.node.name));
                  }
                  else return null; 
                })
                .style("fill-opacity", function(d) { 
                  if (different.checked && same.checked) return null;
                  else if (different.checked) return d.source.node.name === d.target.node.name ? 0 : null;
                  else if (same.checked) return d.source.node.name !== d.target.node.name ? 0 : null;
                  else return 0;
                })
                .style("visibility", function(d) { 
                  return missing.checked ? "visible" : d.target.node.name === "Missing" ? "hidden" : "visible"; 
                })
          }
          else{
       ...
       //   ribbon.exit().remove();
        ...
        function updateCategories(g) {
          ...
          categoryEnter.append("rect")
              ...
              .attr("rx", 4)
              .attr("ry", 4)
              .attr("class", function(d) { return "category-" + ordinal(d.name) })
    ...
    function layout(tree, dimensions, ordinal) {
      var nodes = [],
          ...
          y0 = flip.checked ? height - 2 : 45,
          dy = (height - 45 - 2) / (nd - 1);
        
      dimensions.forEach(function(d, i) {
        var ind = +d.categories.indexOf("Missing");
        if(ind !== -1)
            d.categories = d.categories.splice(ind,1);
        d.categories.forEach(function(c) {
          c.dimension = d;
          c.count = 0;
          c.nodes = [];
        });
        d.y = flip.checked ? y0 - i * dy : y0 + i * dy;
      });
      ...
      function recurse(p, d, depth, major) {
        ...
        dimension.categories.forEach(function(c,i) {
         ...
         /* if(!missing.checked){
            if(child.source.node.name != "Missing" && child.target.node.name=="Missing"){
    ...
    function ribbonPath(d) {
      var s = d.source,
          t = d.target;
      //if(rscale==1) {
      //  return ribbonPathString(s.node.x0 + s.x0-4, s.dimension.y0, s.dx+8, t.node.x0 + t.x0-4, t.dimension.y0, t.dx+8, tension0);
      //}
